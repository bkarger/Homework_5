...
```{r, message=FALSE, warning=FALSE, error=FALSE, fig.height=7, fig.width=5}
...
ggplot() +
  ...
  geom_sf(data = chix1) +
  geom_sf(data = sf_homicides)
  
```
Creating an sf objet from cleaned homicide data.
```{r, message=FALSE, warning=FALSE, error=FALSE}
sf_homicides <- st_as_sf(x = homicides, coords = c("lat", "lon")) %>% 
  st_set_crs(7634)
